...
#define API_KEY "firebase_apikey"
#define DATABASE_URL "https//yourfirebaselink.firebasedatabase.app/"
...
void setup() {
  ...
  Serial.begin(115200);
  // delay(10000); // first time just in case of charging up the capacitor. Then its useless, but 2 seconds not a big differnce overall
  delay(1000); //just for the startup
  ...
  // Volt = (analogRead(pinVolt)*3.3/4095)/(R2/(R1+R2)); // ADC2 pin cannot be used when wifi is on, that's why we read voltage now and not later on
  Serial.println(Volt);
  ...
  //Turn on the BMP80
  digitalWrite(pinVCC, HIGH); // --> we activate here the pinout 
  digitalWrite(LEDpin, HIGH); // --> the led needs to to a different pin, if we use it in series with VCC of BMP280 a drop of 0.6V happens and the BMP does not get detected
  bool status;
  // default settings
...
void loop() {
  ...
  if (Firebase.ready() && signUpOK){
    ...
    ++count;
    Firebase.RTDB.setString(&fbdo, "BMP280_final/" + String(count) + "/Time", printLocalTime());
    Firebase.RTDB.setFloat(&fbdo, "BMP280_final/" + String(count) + "/Temp", bme.readTemperature());
    Firebase.RTDB.setFloat(&fbdo, "BMP280_final/" + String(count) + "/Humd", bme.readHumidity());
    Firebase.RTDB.setFloat(&fbdo, "BMP280_final/" + String(count) + "/Alti", bme.readAltitude(SEALEVELPRESSURE_HPA));
    Firebase.RTDB.setFloat(&fbdo, "BMP280_final/" + String(count) + "/Pres", bme.readPressure()/100.0F);
    Firebase.RTDB.setFloat(&fbdo, "BMP280_final/" + String(count) + "/BVolt", Volt);
    ...
    // bme.reset(); // sends bmp280 to sleep --> apparently from 0.4mA to 8uA. Not bad!
    ...
    BME280_Sleep(); // sends bme280 to sleep. Credits to David Bird 
